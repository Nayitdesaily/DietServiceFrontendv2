...
export default function CategoriaRestauranteModal({ toggle, modal, categoriaSelec, setCategoriaSelec }) {
  ...
  function Submit(data) {
    if(categoriaSelec != null) {
        axios.put(`http://localhost:8000/api/categorias-chef/actualizar/${categoriaSelec?.category_id}`, categoriaSelec).then((res) => console.log(res.data.message));
    } else {
        axios.post("http://localhost:8000/api/categorias-chef/crear", data).then((res) => console.log(res.data.message));
    }
    toggle();
